...
$ignoreFileIndex = Get-Content "C:\Users\d020715\OneDrive\dev\media-importer\out\marked-for-deletion.csv"
if (!($ignoreFileIndex -contains $file)) {
    "Adding ", $file, " to index" -join ' '  | out-file "C:\Users\d020715\OneDrive\dev\media-importer\out\marked.log" -append
    $file | out-file "C:\Users\d020715\OneDrive\dev\media-importer\out\marked-for-deletion.csv" -append
} else {
    $file, " is already in index"  -join ' '  | out-file "C:\Users\d020715\OneDrive\dev\media-importer\out\marked.log" -append
}

# move file to MarkDeleted
$target = ($file -replace "\\Pictures\\", "\\Pictures\\.deleted\\")
"Target is ", $target  -join ' '  | out-file "C:\Users\d020715\OneDrive\dev\media-importer\out\marked.log" -append

if (Test-Path -path $target) {
    "Removing source ", $file -join ' '  | out-file "C:\Users\d020715\OneDrive\dev\media-importer\out\marked.log" -append
    Remove-Item $file
} else {
